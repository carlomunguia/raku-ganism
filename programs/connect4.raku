...
class HumanPlayer is Player {
    ...
    method get_move(Game $game) {
        ...
        loop {
            my $user_input = prompt("Enter a Column number for {$.token} to play: ");
    ...
    method get_move_choices( Game $game, $debug = 1 ) {
        ...
        if $.look_ahead > 0 && @legal_moves.grep: { .is_winning_move } -> @winning_moves {
            ...
            if $.look_ahead > 2 && @ok_moves.grep: { .gives_opponent_only_losing_moves } -> @better_moves {
                say "'{.token}' has better moves & likes to play one of: { @better_moves.map({.column + 1}) }" if $debug > 0;
                return @better_moves;
            } else {
                say "'{$.token}' has ok moves though, and should play one of: { @ok_moves.map({.column + 1}) }" if $debug > 0;
                return @ok_moves;
            }
        }
        else {
            say "'{$.token}' has no moves that are good, no preference" if $debug > 0;
            return @legal_moves;
        }
    }
    method get_move( Game $game ) {
...
class Game {
    ...
    method clear_board() {
        if @.player_types.elems != 2 {
            die "invalid game spec: { @.player_types } -- expected list of two elements, each is either strength or 'H' for human";
        }
...
class Move {
    ...
    method raku () { "Move( :who<($.who.token)>, :col<$.column> :row<$.row> )" };
